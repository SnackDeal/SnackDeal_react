        ...
        const nextCategories = Array.from(
          new Map(
            result.items
              ...
              .map((product) => [
                product.category_id,
                { id: product.category_id, name: product.category },
              ])
          ).values()
        );
    ...
    const loadProducts = async () => {
      ...
      try {
        ...
        if (filters.sort === 'stock_asc') {
          ...
          nextItems.sort(
            (a, b) => Number(a.is_soldout) - Number(b.is_soldout) || b.id - a.id
          );
        }
        ...
        const hydratedVisibleItems = await Promise.all(
          visibleItems.map(async (product) => {
            try {
              return await apiGetProduct(product.id);
            } catch {
              return product;
        ...
        setProducts(hydratedVisibleItems);
        setTotal(result.total);
      } catch (err) {
        console.error('Failed to load products:', err);
        setProducts([]);
        setTotal(0);
        setError((err as { message?: string }).message ?? '상품 목록을 불러오지 못했습니다.');
      } finally {
        setLoading(false);
      }
    };
            ...
            {[{ id: 0, name: '전체' }, ...categories].map((cat) => {
              ...
              return (
                <button
                  key={cat.id}
                  onClick={() =>
                    cat.id === 0 ? handleResetCategory() : handleCategoryClick(cat.id)
                  }
                  style={{
                    padding: '8px 16px',
              ...
              >
                ...
                <div style={{ padding: 14 }}>
                  ...
                  >
                    ...
                    <span
                      style={{ marginLeft: 4, fontSize: 14, color: '#64748b', fontWeight: 600 }}
                    >
                      원
                    </span>
                  </div>
